...
#define READ_BEGIN() [](int i){}}; while (el.type != BSONReader::ElementType::NoMoreData) { \
    ...
    BSONMessage* BSONMessage::ReadMessage(FSocket* sock)
    {
...
#define HELLO_MSG_LEN 0
    ...
    int64_t HelloMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_EPILOGUE();
    }
...
#define PHYSICALPROPERTIES_MSG_LEN 1 * 3 + 3 * 3 + 4
    ...
    int64_t PhysicalPropertiesMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
...
#define VISUALDATAENABLE_MSG_LEN 1
    ...
    int64_t VisualDataEnableMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
...
#define VISUALMETADATAENABLE_MSG_LEN 1
    ...
    int64_t VisualMetaDataEnableMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_ELEMENT(enabled);
        SEND_EPILOGUE();
...
#define VISUALDATA_MSG_LEN 1 + 1 + 3 + 4
    ...
    int64_t VisualDataMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
...
#define BEAM_MSG_LEN 3 * 3 + 5 * 1
    ...
    int64_t BeamMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_VECTOR3(origin);
        SEND_VECTOR3(velocity);
...
#define COLLISION_MSG_LEN 3 * 2 + 1 * 3
    ...
    int64_t CollisionMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_VECTOR3(position);
...
#define SPAWN_MSG_LEN 3 * 3 + 4 + 4 * 1
    ...
    int64_t SpawnMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
...
#define SCANRESULT_MSG_LEN 3 * 3 + 4 + 4 * 1
    ...
    int64_t ScanResultMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_ELEMENT(obj_type);
        SEND_ELEMENT(mass);
...
#define SCANQUERY_MSG_LEN 1 + 1 + 3
    ...
    int64_t ScanQueryMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
...
#define SCANRESPONSE_MSG_LEN 1 + 1
    ...
    int64_t ScanResponseMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_ELEMENT(data);
        SEND_EPILOGUE();
...
#define GOODBYE_MSG_LEN 0
    ...
    int64_t GoodbyeMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_EPILOGUE();
    }
...
#define DIRECTORY_MSG_LEN 4
    ...
    int64_t DirectoryMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
...
#define NAME_MSG_LEN 1
    ...
    int64_t NameMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_ELEMENT(name);
        SEND_EPILOGUE();
...
#define READY_MSG_LEN 1
    ...
    int64_t ReadyMsg::send(FSocket* sock)
    {
        SEND_PROLOGUE();
        SEND_ELEMENT(ready);
        SEND_EPILOGUE();
