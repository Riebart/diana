...
// We can use string literals in templates, because they're constant and known at
// compile-time, but only if we couch it in some ceremony.
//
// This is a template where the size is the number of characters, and this
// template parameter is actually provided based SFINAE; that is, when you
// instantiate a `StringLiteral("abcdef")`, one and only one possible template
// parameter will work, and all others will fail to substitute.
//
// It's a bit of magic, but since it _can_ be instantiated, and everything about
// is constant (const), this struct will expose the string literal as a constexpr
// at compile-time via (because string literals aren't a constexpr, for some
// unknown, and ungodly stupid reason) and thus allow us to use it as a template
// parameter.
template<size_t N>
...
// Wraps the basic type that is the members of the struct, providing some helpful
// functions that are all known at compile-time, specifically:
template <typename T>
struct Element
{
    T value;
    ...
    // Wraps the system hton and ntoh and provides compile-time dispatch based
    // on the _size_ of the member, and not the _type_ of the member, as well
    // as providing 64-bit versions.
    inline void hton()
    {
        __hton<T>(value);
    }
    ...
    // Wraps sizeof for simple types, but can be overloaded for larger complex
    // types, see the implementations for char* and std::string.
    inline std::size_t binary_size() const
    {
        return sizeof(T);
    }

    // Read in an element correctly from output assumed to be generated by
    // binary_write()
    ...
    // Serialize this element to binary, with this implementation providing
    // support for basic non-pointer and non-class types. Any type where the
    // value is entirely contained in the structure should work here.
    //
    // Any value that performs dynamic memory management, or pointer-types
    // will require special handling and a bespoke implementation.
