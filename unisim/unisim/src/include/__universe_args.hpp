...
double opt_collision_energy_cutoff = parser.get_basic_option(
                                               ...
                                               1e-9,
                                               "Collisions (physical or beam) that result in a transfer of energy below this amount are ignored. This helps to ensure that spurious collisions (stiction) are gracefully handled. ", false)
                                         .result.option_value;
...
double opt_gravitational_constant = parser.get_basic_option(
                                              ...
                                              6.67384e-11,
                                              "Universal gravitational constant. ", false)
                                        .result.option_value;
...
double opt_health_damage_threshold = parser.get_basic_option(
                                               ...
                                               0.1,
                                               "During a collision, non-smart objects make take damage equal to one point of health per Joule of collision energy that exceeds this proportion of the object's current health. ", false)
                                         .result.option_value;
...
double opt_health_mass_scale = parser.get_basic_option(
                                         ...
                                         "--health-mass-scale",
                                         1e6,
                                         "Non-smart physics objects are assigned a number of hit points that is their mass multiplied by this value. ", false)
                                   .result.option_value;
...
double opt_max_simultaneous_collision_rounds = parser.get_basic_option(
                                                         ...
                                                         100,
                                                         "Maximum number of rounds of collision simulation in which simultaneous collisions are considered. This value is only going to come into play with precisely placed objects like done in code. True simultaneous collisions are astronomically unlikely to occur in a real simulation scenario. ", false)
                                                   .result.option_value;
...
int32_t opt_network_port = parser.get_basic_option(
                                     ...
                                     "--network-port",
                                     5505,
                                     "TCP port to listen for connections on. ", false)
                               .result.option_value;
...
int32_t opt_num_worker_threads = parser.get_basic_option(
                                           ...
                                           "--num-worker-threads",
                                           1,
                                           "Number of threads to use for service work tasks (physics, vis-data transmission, etc...). Note that this does not control the threads used for TCP clients, as every client receives it's own thread independent of this setting. ", false)
                                     .result.option_value;
params.num_worker_threads = opt_num_worker_threads;
bool opt_permit_spin_sleep = parser.get_flag_option(
                                       "",
                                       "--permit-spin-sleep",
                                       "Permit spin sleeping to slow realtime simulations down to real time. ", false)
                                 .result.option_value;
...
double opt_simulation_rate = parser.get_basic_option(
                                       ...
                                       "--simulation-rate",
                                       1.0,
                                       "When paired with realtime_physics, this controls a simulation rate relative to realtime. Values <1.0 result in simulations that are slower than real time, and values >1.0 result in simulations faster than realtime. ", false)
                                 .result.option_value;
...
double opt_spectrum_slush_range = parser.get_basic_option(
                                            ...
                                            0.01,
                                            "Spectrum power levels are randomly adjusted by this proportion upon receipt by the universe to provide a statistical guarantee that two objects don't have identical signature spectra. If teh power level is below this amount, then it is set to a random value between 0 and this value, inclusive. ", false)
                                      .result.option_value;
...
double opt_speed_of_light = parser.get_basic_option(
                                      ...
                                      "--speed-of-light",
                                      299792458l,
                                      "The speed of light in this universe. The defualt speed for beams, and the speed at which relativistic effects do/would take effect. ", false)
                                .result.option_value;
params.speed_of_light = opt_speed_of_light;
bool opt_verbose_logging = parser.get_flag_option(
                                     "",
                                     "--verbose-logging",
                                     "Print verbose information, such as collision rounds per tick, and when a collision happens, and which objects were involved. ", false)
                               .result.option_value;
